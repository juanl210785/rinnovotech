                            ...
                            <div class="fade-mode">
                                <div class="h-64">
                                    <div class="h-full image-fit rounded-md overflow-hidden">
                                        <img alt="Midone - HTML Admin Template" src="{{ $this->variant->image }}" />
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        <div>
            ...
            <div class="intro-y box p-4 shadow-lg" x-data="{
                ...
                <p class=" text-trueGray-700 mb-4">
                    <span class=" font-semibold text-lg mr-2">{{ __('Available stock') }}:</span>
                    {{ $product->stock }}
                </p>
                                ...
                                @foreach ($option->pivot->features as $feature)
                                    <li>
                                        @switch($option->type)
                                            @case(1)
                                                <button
                                                    class="w-20 h-8 btn {{ $selectedFeatures[$option->id] == $feature['id'] ? 'btn-primary' : 'btn-secondary' }} uppercase font-semibold"
                                            ...
                                            @case(2)
                                                <button
                                                    class="w-20 h-8 btn uppercase font-semibold"
                                                    wire:click="$set('selectedFeatures.{{ $option->id }}', {{ $feature['id'] }})"
                ...
                <div class="flex">
                    <div>
                        <button class="btn btn-primary" x-on:click="qty = qty - 1" x-bind:disabled="qty == 1">
                            -
                        </button>
                        <span x-text="qty" class="inline-block w-6 text-center text-gray-700"></span>
                        <button class="btn btn-primary" x-on:click="qty = qty + 1">
                            +
                        </button>
                    </div>
